...
public class BookLoansDAO extends BaseDAO<BookLoan> {
	...
	public BookLoan get(Integer bookId, Integer branchId, Integer cardNo) throws SQLException {
		List<BookLoan> bookLoans = extractData(read(new Table(tableName).select() + " WHERE bookId=? and branchId=? and cardNo=? and dateIn is null;", new Object[] {
	...
	public List<BookLoan> extractData(ResultSet results) throws SQLException {
		...
		while (results.next()) {
			...
			if (results.getTimestamp("dateIn") != null) {
				bookLoan.setDateIn(LocalDateTime.ofInstant(results.getTimestamp("dateIn").toInstant(), ZoneId.systemDefault()));
			}
			if (results.getTimestamp("dateOut") != null) {
				bookLoan.setDateOut(LocalDateTime.ofInstant(results.getTimestamp("dateOut").toInstant(), ZoneId.systemDefault()));
			}
			if (results.getTimestamp("dueDate") != null) {
				bookLoan.setDueDate(LocalDateTime.ofInstant(results.getTimestamp("dueDate").toInstant(), ZoneId.systemDefault()));
			}
